...
const (
	...
	Sessions_Advance_FullMethodName       = "/agentsessions.v1.Sessions/Advance"
...
type SessionsClient interface {
	...
	Advance(ctx context.Context, in *AdvanceRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[AdvanceUpdate], error)
...
func (c *sessionsClient) Advance(ctx context.Context, in *AdvanceRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[AdvanceUpdate], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Sessions_ServiceDesc.Streams[0], Sessions_Advance_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[AdvanceRequest, AdvanceUpdate]{ClientStream: stream}
...
type Sessions_AdvanceClient = grpc.ServerStreamingClient[AdvanceUpdate]
...
type SessionsServer interface {
	...
	// expects. The live stream carries committed LogRecords plus ephemeral Deltas; Replay
	// re-delivers committed records only (read-only).
	Advance(*AdvanceRequest, grpc.ServerStreamingServer[AdvanceUpdate]) error
...
func (UnimplementedSessionsServer) Advance(*AdvanceRequest, grpc.ServerStreamingServer[AdvanceUpdate]) error {
...
func _Sessions_Advance_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(AdvanceRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(SessionsServer).Advance(m, &grpc.GenericServerStream[AdvanceRequest, AdvanceUpdate]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Sessions_AdvanceServer = grpc.ServerStreamingServer[AdvanceUpdate]
...
var Sessions_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "Advance",
			Handler:       _Sessions_Advance_Handler,
			ServerStreams: true,
		},
		{
			StreamName:    "Replay",
